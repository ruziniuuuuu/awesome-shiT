...
这个仓库是一座以公开 GitHub repo 为实物证据的中文互联网文化档案馆：重点保存烂梗、抽象文化、恶俗玩具、低技术力高节目效果项目，以及少量被公开材料支持的真实工程史样本。

核心任务很简单：在 GitHub 上找史，把值得留档的项目整理成可核验、可发布、可继续生长的 Markdown 史料库。

## 当前架构覆盖说明（2026-09-05）

项目已由双向 Wiki 改为单页编年史档案馆，以下规则优先于本文后续仍提到 Quartz、Obsidian、WikiLinks、wiki/index.md 或 wiki/log.md 的旧说明：

- wiki/repos/、wiki/concepts/ 与 raw/ 是唯一史料源，必须保留。
- site/ 是轻量静态前端；scripts/ 负责从史料生成 public/、校验和更新 GitHub metadata。
- archive/github-snapshot.json 保存可核验的 GitHub 时间快照；archive/cultural-events.json 保存文化节点，无可靠出处时必须标记“待考”。
- 不再维护双向链接、手写总索引、操作日志、Graph、Backlinks、Explorer 或 Quartz 代码。
- 项目与支系的关系只读取项目 frontmatter 的 concepts；修改史料后运行 npm run check 与 npm run build。
- public/ 是可复现的构建产物，不提交。
...
- 具有“低创但荒诞”“严肃实现荒谬目标”“README 本身就是节目”的项目。
